...
%     stair.stimRange         = (linspace(stair.PF([6 2 0 0],.1,'inverse'),stair.PF([6 2 0 0],.9999,'inverse'),17));  % mm obstruction
    ...
    stair.priorBetaRange    = linspace(-0.3,2.1,stair.grain);
    ...
    prior = ...
        PAL_pdfNormal(log(PM.priorAlphas),2.57,0.105).*...
        PAL_pdfNormal(log(10^PM.priorBetas),2.05,0.85).*...
        ones(size(PM.priorGammas)).*...
        ones(size(PM.priorLambdas));
    
    prior=prior./sum(sum(sum(sum(prior))));
    
    stair.PM = PAL_AMPM_setupPM(stair.PM,'prior',prior);
