...
checking these can be done with the following code.
```{r}
...
sp <- get_sampler_params(fit_eip_HRDT$fit, inc_warmup = FALSE)

df <- do.call(rbind, lapply(seq_along(sp), function(i) {
  cbind(as.data.frame(sp[[i]]), chain = i)
}))


df %>% group_by(chain) %>% summarize(number_divergent = sum(divergent__),
                                     low_energy = bfmi(energy__) < 0.3,
                                     max_treedpeth = max(treedepth__))

```

Here we want to see that the number of divergences for each chain is 0 that the low_energy is FALSE for all chains and the max_treedepth is below what we set when running the model (i.e. 10). 
...
empirically_informed_priors_RRST = c(
  set_prior("normal(2.57,0.105)", class="b", nlpar="alpha", coef = "Intercept"),
  set_prior("normal(0,0.105)", class="b", nlpar="alpha", coef = "conditiontreatment"),
  set_prior("normal(2.05,0.85)", class="b", nlpar="beta", coef = "Intercept"),
  set_prior("normal(0,0.85)", class="b", nlpar="beta", coef = "conditiontreatment"),
  set_prior("normal(-4.51,2.53)", class="b", nlpar="lambda"),
...
sp <- get_sampler_params(fit_eip_RRST$fit, inc_warmup = FALSE)
