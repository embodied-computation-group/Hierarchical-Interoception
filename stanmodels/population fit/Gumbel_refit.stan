...
model{
  ...
  gm[1] ~ normal(-1.9,1.3); //global mean of beta
  ...
  Y ~ binomial(npx, lapse + (1 - 2 * lapse) .* (1-exp(-10^(beta .* (X-alpha)))));
...
generated quantities{
  ...
  vector[T] log_lik;
    ...
    for (n in 1:T){
    log_lik[n] = binomial_lpmf(Y[n] |npx[n], (inv_logit(dot_product(X_lapse[,n], lapse_p[,S_id[n]])) / 2) + (1 - 2 * (inv_logit(dot_product(X_lapse[,n], lapse_p[,S_id[n]])) / 2)) * (1-exp(-10^(exp((X_beta[1,n] * param[S_id[n],1] + X_beta[2,n] * param[S_id[n],2])) * (X[n]-(X_alpha[1,n] * param[S_id[n],3] + X_alpha[2,n] * param[S_id[n],4]))))));
