...
transformed parameters{
  ...
  alpha = exp(gm[1]+(tau_u[1] * z_expo[1,]))';  
...
model{
  ...
  for (n in 1:N){
    Y[n] ~ bernoulli(lapse[S_id[n]] + (1 - 2 * lapse[S_id[n]]) * (1-exp(-10^(beta[S_id[n]]*(X[n,2]-alpha[S_id[n]])))));
  }
}
...
generated quantities{
  ...
  for (n in 1:N){
    log_lik[n] = bernoulli_lpmf(Y[n] |lapse[S_id[n]] + (1 - 2 * lapse[S_id[n]]) * (1-exp(-10^(beta[S_id[n]]*(X[n,2]-alpha[S_id[n]])))));
