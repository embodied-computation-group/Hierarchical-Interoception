...
threshold = combined %>% 
  ...
  geom_abline(aes(intercept = 0,slope=1))+
  geom_linerange(aes(x=simulated_values,ymin=q5,ymax=q95,color=dataset))+
  geom_text(aes(x=100,y=-50+30*as.numeric(dataset=='gaussian'),label=sprintf("%.2f [%.2f - %.2f]",round(recovery_rate,2),round(lb,2),round(ub,2)),color=dataset))+
  theme_minimal()+
  scale_color_manual(values = c('#0072B2','#E69F00'))+
  theme(legend.position = 'none')+
  xlab('Ground truth')+
...
slope = combined %>% 
  ...
  geom_abline(aes(intercept = 0,slope=1))+
  geom_linerange(aes(x=simulated_values,ymin=q5,ymax=q95,color=dataset))+
  geom_text(aes(x=6.5,y=2.5+1*as.numeric(dataset=='gaussian'),label=sprintf("%.2f [%.2f - %.2f]",round(recovery_rate,2),round(lb,2),round(ub,2)),color=dataset))+
  theme_minimal()+
  scale_color_manual(values = c('#0072B2','#E69F00'))+
  theme(legend.position = 'none')+
  xlab('Ground truth')+
...
lapse = combined %>% 
  ...
  geom_abline(aes(intercept = 0,slope=1))+
  geom_linerange(aes(x=simulated_values,ymin=q5,ymax=q95,color=dataset))+
  geom_text(aes(x=0.45,y=0.2+0.07*as.numeric(dataset=='gaussian'),label=sprintf("%.2f [%.2f - %.2f]",round(recovery_rate,2),round(lb,2),round(ub,2)),color=dataset))+
  theme_minimal()+
  scale_color_manual(values = c('#0072B2','#E69F00'))+
  theme(legend.position = 'none')+
  xlab('Ground truth')+
...
ggsave(here::here("figures",'supp_ss_recov.tiff'),units = 'in',width = 7.2,height = 7.2)
