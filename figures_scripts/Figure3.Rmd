...
combined<-
  combined%>% mutate(simulated_values = NULL) %>% 
  filter(model==dataset) %>% 
  full_join(truth) %>% 
...
truth_gaussian <- read_csv(here::here("simulated_data","model_parameter_recovery","subject_level",
                                      "R_gaussian_50_80_all_participant_information.csv")) %>% 
  group_by(idx) %>% 
  mutate(id = 1:n())

truth_gaussian$dataset="gaussian"

truth_gumbell <- read_csv(here::here("simulated_data","model_parameter_recovery","subject_level",
                                     "R_gumbel_50_80_all_participant_information.csv")) %>% 
  group_by(idx) %>% 
  mutate(id = 1:n())
...
truth<-
  full_join(truth_gaussian,truth_gumbell) %>% 
  pivot_longer(cols = c(alpha,beta,lambda),names_to = 'variable') %>% 
  select(variable,value,idx,dataset,id) %>% 
  rename(simulated_values = value) %>% 
  mutate(variable = ifelse(variable == "alpha","threshold",ifelse(variable == "beta","slope",ifelse(variable == "lambda","lapse",NA))))
...
temp <-
  combined%>% mutate(id = as.numeric(id)) %>% 
  filter(model==dataset) %>% 
  select(-c(model,models)) %>%
  full_join(truth) %>% 
  filter(meanrhat<=1.01,mean_div==0) %>% 
  mutate(recovered=simulated_values>q5&simulated_values<q95)
...
threshold = combined %>% 
  ...
  geom_abline(aes(intercept = 0,slope=1))+
  geom_linerange(aes(x=simulated_values,ymin=q5,ymax=q95,color=dataset))+
  geom_text(aes(x=-100,y=100+50*as.numeric(dataset=='gaussian'),label=sprintf("%.2f [%.2f - %.2f]",round(recovery_rate,2),round(lb,2),round(ub,2)),color=dataset))+
  theme_minimal()+
  scale_color_manual(values = c('#0072B2','#E69F00'))+
  theme(legend.position = 'none')+
  xlab('Ground truth')+
  ylab('Estimate')+
  ggtitle('Subject level threshold')


# ggsave(here::here("figures",'ss_Figure3B.tiff'),units = 'cm',width = 8,height = 6.5)

slope = combined %>% 
  filter(variable=='slope') %>% 
  ggplot()+
  geom_abline(aes(intercept = 0,slope=1))+
  geom_linerange(aes(x=log(simulated_values),ymin=log(q5),ymax=log(q95),color=dataset))+
  geom_text(aes(x=-4.5,y=1+1.5*as.numeric(dataset=='gaussian'),label=sprintf("%.2f [%.2f - %.2f]",round(recovery_rate,2),round(lb,2),round(ub,2)),color=dataset))+
  theme_minimal()+
  scale_color_manual(values = c('#0072B2','#E69F00'))+
  theme(legend.position = 'none')+
  xlab('Ground truth')+
  ylab('Estimate')+
  ggtitle('Subject level slope')


# ggsave(here::here("figures",'ss_Figure3C.tiff'),units = 'cm',width = 8,height = 6.5)

lapse = combined %>% 
  filter(variable=='lapse') %>% 
  ggplot()+
  geom_abline(aes(intercept = 0,slope=1))+
  geom_linerange(aes(x=brms::logit_scaled(simulated_values),ymin=brms::logit_scaled(q5),ymax=brms::logit_scaled(q95),color=dataset))+
  geom_text(aes(x=-11,y=-0.5+2*as.numeric(dataset=='gaussian'),label=sprintf("%.2f [%.2f - %.2f]",round(recovery_rate,2),round(lb,2),round(ub,2)),color=dataset))+
  theme_minimal()+
  scale_color_manual(values = c('#0072B2','#E69F00'))+
  theme(legend.position = 'none')+
  xlab('Ground truth')+
  ylab('Estimate')+
  ggtitle('Subject level lapse')

# ggsave(here::here("figures",'ss_Figure3D.tiff'),units = 'cm',width = 8,height = 6.5)
```
...
ggsave(here::here("figures",'supp_ss_recov_scale.tiff'),units = 'in',width = 7.2,height = 7.2)
