...
```{r}
fit <- readRDS(here::here("results","Fit population","RRST_gumbelpriors_n_centered_3000_0.9.RDS"))
library(tidyverse)
library(mvtnorm)
```
...
for(draw in 1:M){
  ...
  draws_hp$alpha[draw]=d[1]
  draws_hp$beta[draw]=exp(d[2])
  draws_hp$lambda[draw]=0.5/(1+exp(-d[3]))
}

g<-expand.grid(.draw=1:M,x=log(seq(0.1,17,.1)))

pfs_ppc<-
  draws_hp %>% 
  mutate(mu_lambda=0.5/(1+exp(-mu_lambda)),mu_beta=exp(mu_beta),mu_alpha=(mu_alpha)
         ) %>% 
  full_join(g) %>% 
     mutate(
    x_pct = (exp(x) / 17) * 100
  ...
  mutate(
    g=0.5+(0.5-mu_lambda)*(1-exp(-10^(mu_beta * (x-mu_alpha)))),
    s=0.5+(0.5-lambda)*(1-exp(-10^(beta*(x-alpha)))),
    ) %>% 
