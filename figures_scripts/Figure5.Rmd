...
```{r}
fit <- readRDS(here::here("results","Fit population","RRST_weibull_priors.RDS"))
library(tidyverse)
library(mvtnorm)
```
...
for(draw in 1:M){
  omega<-
    ...
    cor()
  tau<-c(
    draws_hp$tau_alpha[draw],
    draws_hp$tau_beta[draw],
    draws_hp$tau_lambda[draw]
    )
  sigma<-diag(tau)%*%omega%*%diag(tau)
  mu<-c(
    draws_hp$mu_alpha[draw],
    draws_hp$mu_beta[draw],
    draws_hp$mu_lambda[draw]
    )
...
g<-expand.grid(.draw=1:M,x=seq(0.1,17,.1))
...
pfs_ppc<-
  ...
  mutate(
    g=0.5+(0.5-mu_lambda)*(1-exp(-1*(x/mu_alpha)^mu_beta)),
    s=0.5+(0.5-lambda)*(1-exp(-1*(x/alpha)^beta)),
    ) %>% 
  select(g,s,x_pct,.draw,x) %>%  
  group_by(x_pct,x) %>%
  summarize(
...
pfs_ppc %>% 
  ggplot()+
  geom_ribbon(aes(x=x_pct,ymin = s_lb,ymax=s_ub,alpha=ci),fill=rgb(118/255,42/255,131/255))+
  geom_line(aes(x=x_pct,y=g,linetype=type),linewidth=1,color=rgb(118/255,42/255,131/255))+
...
ggsave('Figure5.png',units = 'cm',width = 16,height = 8)
```