...
This demo is a companion of our paper : INSERT_DOI

# Analyzing Heart Rate Discrimination Task data

## Data simulation (optional)

To demonstrate how to run the hierarchical analysis in BRMS, we first need some data. Therefore, we simulate 50 subjects going through two conditions of a task similar to the HRDT, with the treatment condition leading to an increase of the threshold parameter alpha of 5 ΔBPM.
...
Now in order to fit this data using BRMS we need to specify a model using *bf()*. In line with the paper, HRDT data is modelled by a Gaussian cumulative density function whose lower and upper asymtotes are set by a lapse rate lambda.
...
*Note: This might take a while. To reduce run time, consider commenting out one of the calls to the brm function.*
...
We can then construct plots that show the posterior distribution for the group-level differences in threshold and slope. We will also add a linerange materializing the 95% CI.
...
as_draws_df(fit_eip_HRDT) %>% 
  ...
  labs(
    title='HRDT parameters group mean differences between conditions',
    x='Treatment-Control',
    y='')+
...
pfs_ppc %>%
  ...
  ggtitle("RRST group mean psychometric functions by condition")
```
...
t %>% 
  ...
  geom_point(data = filter(sim_data_RRST,condition=='control'), aes(x = x, y = y/n,),alpha=.2,col="#56b4e9")+
  facet_wrap(~subj)+
  theme_classic()+
  labs(
...
t %>% 
  ...
  geom_point(data = filter(sim_data_RRST,condition=='treatment'), aes(x = x, y = y/n,),alpha=.2,col="#009E73")+
  facet_wrap(~subj)+
  theme_classic()+
  labs(
    title='Individual participants data and best fitting psychometric functions',
...
This section largely follows the same steps as the one of HRDT data analysis. The text and procedures are largely redundant on purpose.

## Data simulation (optional)

To demonstrate how to run the hierarchical analysis in BRMS, we first need some data. Therefore, we simulate 50 subjects going through two conditions of a task similar to the RRST, with the treatment condition leading to a decrease of the threshold parameter alpha of 1 arbitrary occlusion level.
...
mu_alpha_dif_RRST = log(exp(mu_alpha_RRST)-1)-mu_alpha_RRST
mu_beta_dif_RRST = 0
...
This makes effects on parameter beta difficult to interpret. Instead of testing effects on beta, we therefore recommend to test effects on the spread of the function. This quantity reflects the true dispersion of the function and can be interpreted in terms of precision/reliability similarly to the slope parameter of a Gaussian PF. Here is it's equation for the spread of a Weibull function: 
$$
...
In the case of our simulated data, alpha changes but not beta and the spread is therefore different between the conditions. We can compute the spread difference based on the parameters used to simulate the data:
...
We can also use the posterior samples from our model to estimate the spread difference and assess if it is "significant". Here is some code to compute the posterior mean spread and corresponding 95% CI as well as the posterior probability of the spread difference being larger than 0 and a pseudo p-value for a two-sided test (i.e., H0 is that the effect is 0).
...
The estimate is not far off but the uncertainty is too high to declare a "significant" difference of spread.
...
as_draws_df(fit_eip_RRST) %>% 
  ...
  labs(
    title='RRST parameters group mean differences between conditions',
    x='Treatment-Control',
    y=''
    )+
  facet_wrap(.~name,scale='free')+
...
Finally, we should also have a look at individual participants' psychometric functions to check they align with the data.
```{r}
pf <- function(x,alpha,beta,lambda){inv_logit(lambda) / 2 + (1-inv_logit(lambda)) * (0.5+0.5 * erf(exp(beta)*(x-alpha)/(sqrt(2))))}
